...
const SearchUser = (props) => {
  ...
  const dropdownRef = useRef(null);
    const { projectId, classId, handleClassUserSelection } = props;
  ...
  const handleUserClick = async (selectedUser) => {
    ...
    try {
     ...
     props.handleClassUserSelection(selectedUser);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error('class user:', res.data.users);
  ...
  return (
    ...
    >
      ...
      {console.log('Rendering:', isResultsVisible, userData, userData.length > 0)}
